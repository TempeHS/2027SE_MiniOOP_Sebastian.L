...
## Read an analogue sensor and control the LEDs

| Analogue Sensor Pin | Pico pin               |
| ------------------- | ---------------------- |
| Ground (-, GND, G)  | Analogue Ground (AGND) |
...
> [!Important]
> An analogue sensor must be connected to both an ADC (analogue-to-digital converter) pin and the dedicated Analogue Ground pin.

> [!Note]
> The image uses a potentiometer which is a simple variable resistor that applies no resistance at the off position and maximum resistance at the on position with variable resistance in between. The potentiometer can be replaced by any analogue sensor.
...
2. The analogue read value of the sensor should print to the terminal.
