...
## Read an analogue sensor and control a servo motor

| Servo Motor Pin | Pico pin              |
| --------------- | --------------------- |
| Ground (brown)  | Analogue Ground (GND) |
| Data (amber)    | GP10                  |
| Power (red)     | 5V (VBUS)             |
...
> [!Important]
> There are two types of servo motors, positional and continuous. A positional servo motor (usually 180° or 270°) will position itself based on the PWM signal. So a 500µs signal is position 0 and a 2500µs signal is the nth-last position (180 or 270). A continuous servo is a DC motor with a controller, with the speed and direction being set by the signal. So a 500µs signal is the fastest speed in reverse, a 1500µs signal is stop and a 2500µs signal is the fastest speed forward.
...
2. The brightness of both the onboard LED and external LED should be responsive to the analogue sensor.
3. The servo should sweep 180° in response to the analogue sensor.
