...
> [!Note]
> An ultrasonic sensor sends out an ultrasonic wave from the `trig` pin then listens for the wave to bounce back on the `echo` pin. It uses time and the speed of travel of an ultrasonic wave divided in half to calculate the approximate distance an object is away. They are a cheap solution but they are affected by complex surfaces, environmental changes (temperature and humidity), and their own noise.
